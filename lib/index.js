...
module.exports = {
  ...
  allowedOptions: ['enableLogging', 'level', 'services'],
  ...
  addOptions(options) {
    ...
    for (let key in options) {
      if (this.allowedOptions.indexOf(key)) {
        let that = this;
        switch(key) {
          case 'enableLogging':
          ...
          case 'level':
            ...
            if (typeof(options[key]) != 'object') throw `Type exception on parameter options ${key}, must be object (got ${typeof(key)})`;
