      ...
      if (key === 'enableLogging' || 'level' || 'services') {
        switch(key) {
          ...
          case 'services':
            ...
            if(typeof(options.services) != 'array') throw `Type exception on parameter options ${key}, must be array (got ${typeof(options.services)})`
            ...
            for (let i=0; i < options.services.lenght; i++) {
              ...
              this.services.push(options.services[i])
            }
            break
        }
      } else {
